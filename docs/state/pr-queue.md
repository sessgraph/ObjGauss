...
| PR-02C Trainer/Baselines | `c2_implemented_pending_clean_acceptance` | C0 `fc20023` clean supported；C1 代码承载 SHA `adb1a62` clean supported；C2 已实现 sanitized bundle、copy-state/constant-velocity、120 predictions、独立 18-check verifier 与 clean gate，dirty canonical/reverse diagnostic semantic index `17488a15…7c647` 一致 | 先提交 C2 并运行 `./scripts/check-pr02c-baselines`；通过前不得开始 C3 learned arms |
