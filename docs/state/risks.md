...
| R-002 | 当前默认对象分组不是端到端语义级对象分割 | 对象边界和语义一致性不稳定 | 已落地 `DEMO-001`：真实 Plush splat + Object Field + 多视角 mask voting + 前端对象编辑闭环可复现；默认 KMeans 仍保留为 baseline | 仓库内可对真实小场景生成 SAM / CLIP mask manifest，并与 KMeans 基线对比 | open |
...
| R-008 | SAM / CLIP 仍是外部预计算 mask 来源 | 语义分割效果依赖外部脚本，仓库不能端到端复现实例分割 | 已定义稳定 mask manifest 和 `vote-masks` 消费命令 | 可选 SAM / CLIP 生成器接入并记录依赖、许可、权重策略 | open |
