...
| R-018 | Semantic / teacher evidence 可能泄漏 GT identity | 如果 semantic feature 由 `physical_identity_label`、`target_assignment`、oracle object id 或测试标签 one-hot 构造，identity retrieval@1 可能只是答案泄漏，不证明 inference-time evidence 支撑 ObjectState identity | `OBJECTSTATE-IDENTITY-GATE-POLICY-SCOPING-001` 已把当前 `candidate_ready` 限定为 policy=`semantic`，并明确 native Gaussian long-training gate blocked；`OBJECTSTATE-TEACHER-EVIDENCE-CONTRACT-001` 已定义 `TeacherEvidenceBatch`、required provenance、allowed / inference-time sources、forbidden GT / identity provenance keys 和 training permission gate；`OBJECTSTATE-TEACHER-EVIDENCE-LEAKAGE-AUDIT-001` 已实现 semantic shuffle、random semantic baseline、physical label ban 和 train/test semantic source split，并把默认 synthetic one-hot semantic fixture 继续 blocked 在训练门外 | 至少一个真实 inference-time teacher source batch 以 `direct_object_id_embedding_shared=false` 通过 leakage audit，且负向 semantic shuffle / random semantic baseline 不能伪装成 candidate-ready evidence | open |
...
R-017 update 2026-07-09: `OBJECTSTATE-REAL-IDENTITY-ROWS-001`
新增 `objgauss-objectstate-real-identity-rows-v1` 和
`real-identity-rows` CLI，可把 real evidence bundle 中的 `identity` gate accounting rows
转成 existing reality gate identity rows，并运行 identity-only reality gate。`pass` / `fail`
进入真实 pass/fail accounting，`evidence_incomplete` / `unsupported` 保持 blocked，不被
算作模型失败；非 blocked identity rows 继续要求 timestamped identity GT 和
`idf1` / `fragmentation_rate` / `swap_rate` / `identity_collapse`。该缓解只完成 Stage 1
