...
def _evaluate_identity_candidate(
    ...
    *,
    ...
    return {
        "name": name,
        "frame0": {
            "projection": object_state_projection_summary(projection0),
        ...
        "frame1": {
            "projection": object_state_projection_summary(projection1),
...
def _identity_embeddings(
    ...
    for identity in identities:
        ...
        # Confidence is a bounded reliability signal and mass_fraction is
        # support, not identity content.  Including either in the retrieval
        # vector lets object size or occlusion level masquerade as identity;
        # after the confidence ABI normalization they are also the same value.
        embeddings[int(identity)] = np.asarray(
            state.feature,
            dtype=np.float32,
        ).copy()
    return embeddings
