...
ASSETS: tuple[AssetSource, ...] = (
    ...
    AssetSource(
        ...
        status="已接入",
        priority="P0",
        source_url="https://huggingface.co/cakewalk/splat-data/blob/main/nike.splat",
        download_url="https://huggingface.co/cakewalk/splat-data/resolve/main/nike.splat",
        license="来源许可混合，仅用于本地测试",
        formats=(".splat", ".ply", "object_id"),
        best_for="小体积真实 Gaussian cloud demo，用于重新整理 viewer 样例入口和 source splat 对象编辑 smoke。",
        local_path="/samples/nike_objects.ply",
        splat_path="/samples/nike.splat",
        raw_file_name="nike.splat",
        output_file_name="nike_objects.ply",
        default_clusters=4,
        pull_pipeline="splat-to-objgauss-ply",
        pipeline_stage="Demo 可用",
        use_cases=("Demo预览", "真实Splat", "对象编辑"),
    ),
    AssetSource(
        id="cakewalk-room-3dgs-local",
        name="Cakewalk Room 现成 3DGS 场景",
        category="现成 Gaussian 候选",
        source_type="gaussian",
        status="可拉取未接入默认",
        priority="P0",
        source_url="https://huggingface.co/cakewalk/splat-data/blob/main/room.splat",
        download_url="https://huggingface.co/cakewalk/splat-data/resolve/main/room.splat",
        license="来源许可混合，仅用于本地研究审计",
        formats=(".splat", ".ply", "object_id"),
        best_for="室内静态 Gaussian scene；适合本地 object clustering、viewer load 和 cross-sample 负例扩展。",
        local_path="/samples/room_objects.ply",
        splat_path="/samples/room.splat",
        raw_file_name="room.splat",
        output_file_name="room_objects.ply",
        default_clusters=8,
        pull_pipeline="splat-to-objgauss-ply",
        pipeline_stage="可拉取候选",
        use_cases=("静态场景扩展", "管线烟测", "对象分组审计"),
    ),
    AssetSource(
        id="cakewalk-train-3dgs-local",
        name="Cakewalk Train 现成 3DGS 场景",
        category="现成 Gaussian 候选",
        source_type="gaussian",
        status="可拉取未接入默认",
        priority="P0",
        source_url="https://huggingface.co/cakewalk/splat-data/blob/main/train.splat",
        download_url="https://huggingface.co/cakewalk/splat-data/resolve/main/train.splat",
        license="来源许可混合，仅用于本地研究审计",
        formats=(".splat", ".ply", "object_id"),
        best_for="小型现成 .splat；适合快速 pipeline smoke、source splat 加载和对象层生成实验。",
        local_path="/samples/train_objects.ply",
        splat_path="/samples/train.splat",
        raw_file_name="train.splat",
        output_file_name="train_objects.ply",
        default_clusters=5,
        pull_pipeline="splat-to-objgauss-ply",
        pipeline_stage="可拉取候选",
        use_cases=("静态场景扩展", "管线烟测", "快速样例"),
    ),
    AssetSource(
        id="cakewalk-truck-3dgs-local",
        name="Cakewalk Truck 现成 3DGS 场景",
        category="现成 Gaussian 候选",
        source_type="gaussian",
        status="可拉取未接入默认",
        priority="P1",
        source_url="https://huggingface.co/cakewalk/splat-data/blob/main/truck.splat",
        download_url="https://huggingface.co/cakewalk/splat-data/resolve/main/truck.splat",
        license="来源许可混合，仅用于本地研究审计",
        formats=(".splat", ".ply", "object_id"),
        best_for="单一车辆主体静态场景；适合 object proposal、bbox picking 和 segmentation quality 审计。",
        local_path="/samples/truck_objects.ply",
        splat_path="/samples/truck.splat",
        raw_file_name="truck.splat",
        output_file_name="truck_objects.ply",
        default_clusters=4,
        pull_pipeline="splat-to-objgauss-ply",
        pipeline_stage="可拉取候选",
        use_cases=("静态场景扩展", "对象分组审计", "viewer压力小样本"),
    ),
    AssetSource(
        id="cakewalk-garden-3dgs-local",
        name="Cakewalk Garden 现成 3DGS 场景",
        category="现成 Gaussian 候选",
        source_type="gaussian",
        status="可拉取未接入默认",
        priority="P1",
        source_url="https://huggingface.co/cakewalk/splat-data/blob/main/garden.splat",
        download_url="https://huggingface.co/cakewalk/splat-data/resolve/main/garden.splat",
        license="来源许可混合，仅用于本地研究审计",
        formats=(".splat", ".ply", "object_id"),
        best_for="Mip-NeRF360 风格户外静态场景；适合 renderer/LOD 压力、object-field robustness 和 cross-sample 负例。",
        local_path="/samples/garden_objects.ply",
        splat_path="/samples/garden.splat",
        raw_file_name="garden.splat",
        output_file_name="garden_objects.ply",
        default_clusters=10,
        pull_pipeline="splat-to-objgauss-ply",
        pipeline_stage="可拉取大场景候选",
        use_cases=("静态场景扩展", "LOD压力", "对象分组审计"),
    ),
    AssetSource(
        id="cakewalk-bicycle-3dgs-local",
        name="Cakewalk Bicycle 现成 3DGS 场景",
        category="现成 Gaussian 候选",
        source_type="gaussian",
        status="可拉取未接入默认",
        priority="P1",
        source_url="https://huggingface.co/cakewalk/splat-data/blob/main/bicycle.splat",
        download_url="https://huggingface.co/cakewalk/splat-data/resolve/main/bicycle.splat",
        license="来源许可混合，仅用于本地研究审计",
        formats=(".splat", ".ply", "object_id"),
        best_for="户外自行车静态场景；适合复杂前景/背景分离、bbox picking 和 renderer pressure smoke。",
        local_path="/samples/bicycle_objects.ply",
        splat_path="/samples/bicycle.splat",
        raw_file_name="bicycle.splat",
        output_file_name="bicycle_objects.ply",
        default_clusters=10,
        pull_pipeline="splat-to-objgauss-ply",
        pipeline_stage="可拉取大场景候选",
        use_cases=("静态场景扩展", "LOD压力", "复杂前景背景"),
    ),
    AssetSource(
        id="cakewalk-stump-3dgs-local",
        name="Cakewalk Stump 现成 3DGS 场景",
        category="现成 Gaussian 候选",
        source_type="gaussian",
        status="可拉取未接入默认",
        priority="P2",
        source_url="https://huggingface.co/cakewalk/splat-data/blob/main/stump.splat",
        download_url="https://huggingface.co/cakewalk/splat-data/resolve/main/stump.splat",
        license="来源许可混合，仅用于本地研究审计",
        formats=(".splat", ".ply", "object_id"),
        best_for="自然物体/背景混合静态场景；适合 object proposal 失败分析和 segmentation robustness 负例。",
        local_path="/samples/stump_objects.ply",
        splat_path="/samples/stump.splat",
        raw_file_name="stump.splat",
        output_file_name="stump_objects.ply",
        default_clusters=10,
        pull_pipeline="splat-to-objgauss-ply",
        pipeline_stage="可拉取大场景候选",
        use_cases=("静态场景扩展", "对象分组负例", "LOD压力"),
    ),
    AssetSource(
        id="cakewalk-treehill-3dgs-local",
        name="Cakewalk Treehill 现成 3DGS 场景",
        category="现成 Gaussian 候选",
        source_type="gaussian",
        status="可拉取未接入默认",
        priority="P2",
        source_url="https://huggingface.co/cakewalk/splat-data/blob/main/treehill.splat",
        download_url="https://huggingface.co/cakewalk/splat-data/resolve/main/treehill.splat",
        license="来源许可混合，仅用于本地研究审计",
        formats=(".splat", ".ply", "object_id"),
        best_for="大面积自然场景；适合 LOD/streaming pressure 和 no-object static negative evidence。",
        local_path="/samples/treehill_objects.ply",
        splat_path="/samples/treehill.splat",
        raw_file_name="treehill.splat",
        output_file_name="treehill_objects.ply",
        default_clusters=10,
        pull_pipeline="splat-to-objgauss-ply",
        pipeline_stage="可拉取大场景候选",
        use_cases=("静态场景扩展", "LOD压力", "对象分组负例"),
    ),
    AssetSource(
    ...
    AssetSource(
        ...
        best_for="许可干净的单对象家具 Gaussian demo，用于商业展示路线、对象隔离和删除预览。",
        local_path="/samples/polyhaven_chair_demo_objects.ply",
        splat_path="/samples/polyhaven_chair_demo.splat",
        raw_file_name="polyhaven-chair-splatfacto-smoke-sam8f-slots6-benchmark",
        output_file_name="polyhaven_chair_demo_objects.ply",
        pull_pipeline=None,
        pipeline_stage="CC0 训练输出接入",
        use_cases=("商用展示", "对象编辑", "Spark SH route"),
    ),
    AssetSource(
