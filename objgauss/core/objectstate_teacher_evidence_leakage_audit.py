...
def objectstate_teacher_evidence_leakage_audit_summary(
    ...
    *,
    ...
    shuffled = _semantic_benchmark(
        _shuffle_frame1_semantic_features(scenario_list),
    ...
    random_semantic = _semantic_benchmark(
        _random_semantic_features(scenario_list, seed=seed + 202),
    ...
    checks = {
        "physical_label_ban": _physical_label_ban_check(batch_reports),
...
def _object_feature_shift(features: np.ndarray | None, labels: np.ndarray) -> np.ndarray:
    ...
    representative = {
        identity: matrix[np.nonzero(labels_array == identity)[0][0]]
        for identity in identities
    }
    shifted = matrix.copy()
    for index, identity in enumerate(identities):
        source_identity = identities[(index + 1) % len(identities)]
        shifted[labels_array == identity] = representative[source_identity]
...
def _default_synthetic_teacher_batch(
    ...
    return TeacherEvidenceBatch(
        sample_id=f"{scenario.scenario_id}:synthetic-semantic-reference",
        gaussian_ids=tuple(f"{scenario.scenario_id}:t0:g{index:06d}" for index in range(features.shape[0])),
        ...
        provenance={
            ...
            "input_refs": [f"fixture://{scenario.scenario_id}/frame0"],
