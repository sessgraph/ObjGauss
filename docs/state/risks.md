...
| Dirty worktree 把旧 HEAD 冒充最终 lineage | mitigated_local | spec 只冻结 source-commit policy；runtime 注入当前 HEAD；accept/builder/verifier 都要求 clean checkout | 最终 SHA 的本地与远端 delivery supported |
| 重试选择性删除科学失败 | active | attempt 独立保留；科学失败不可重试或换 seed | attempt audit 与 5% 总额外上限通过 |
| Demo 被误当机器证据 | mitigated_local | Demo 只消费已审计 episode；report 为事实源；浏览器重验 report 与 artifact checksum | 最终 SHA 的浏览器行为测试与远端 delivery supported |
