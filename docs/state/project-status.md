...
1. RENDER-005O: 将 `pixel-output-only` 继续拆成 pixel-compute-only / display-only / tiny-pixel-output probes，区分 pixel accumulation shader、pixel storage write 和 fullscreen display pass；同时在 WebGPU-capable 桌面 Chrome 中重跑 Plush / Lego first-frame runtime audit。
