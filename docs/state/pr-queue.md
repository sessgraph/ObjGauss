...
| PR-02C Trainer/Baselines | `c3_implemented_pending_clean_acceptance` | C0 `fc20023`、C1 `adb1a62`、C2 `9ea2b92` clean supported；C3 工作区实现 minimal Object GNN、两个 learned arms、trainer/lineage/独立 verifier；CPU tiny 与 GPU diagnostic repeat 24/24 checks supported，semantic index `709f6f76…d3db`，mutation/test split 被拒绝 | 实现尚未提交；取得提交授权后提交，再运行 clean `./scripts/check-pr02c-trainer`；不运行 HPO/formal test |
