...
1. RENDER-005N: 将 ObjGauss WebGPU runtime path 拆成 accumulation-only / resolve-only / pixel-output-only probes，隔离 `A valid external Instance reference no longer exists` 出现在哪个 pass；同时在 WebGPU-capable 桌面 Chrome 中重跑 Plush / Lego first-frame runtime audit。
