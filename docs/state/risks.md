...
| R-002 | 当前默认对象分组不是语义级对象分割 | 对象边界和语义一致性不稳定 | 已落地 `SEG-001`：预计算 SAM / CLIP / 2D mask 可投票到 `object_logits` 并导出 `object_id`；默认 KMeans 仍保留为 baseline | 仓库内可对真实小场景生成 mask manifest，并与 KMeans 基线对比 | open |
