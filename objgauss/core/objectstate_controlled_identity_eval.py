...
def evaluate_objectstate_controlled_identity_predictions(
    ...
    *,
    ...
    metrics = _identity_metrics(checked_capture, gt_pairs, prediction_map)
...
def _identity_metrics(
    ...
    for frame_id, object_id in gt_pairs:
        ...
        if predicted is None:
            ...
            correct += 1
    fragmentation_changes = 0
    fragmentation_denominator = 0
    for identities in predictions_by_object.values():
        if len(identities) >= 2:
            fragmentation_denominator += len(identities) - 1
            fragmentation_changes += sum(
                1
    ...
    return {
        ...
        "fragmentation_rate": (
            fragmentation_changes / fragmentation_denominator
