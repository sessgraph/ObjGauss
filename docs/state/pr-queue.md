...
| PR-02C Trainer/Baselines | `c6_spec_frozen` | C0 `fc20023`、C1 `adb1a62`、C2 `9ea2b92` clean supported；C3 `4498bd6` GPU golden 24/24 checks supported，semantic index `709f6f76…d3db`；`4498bd6`/`1c0d6ed` 已 push，但尚无 PR-02C CPU workflow；C6 机器 contract 冻结 24 tasks / 12 fairness pairs | 实现 runner、独立 selector、CPU workflow 与负例；在 clean runner commit 只生成一次共享 `hpo_data_index` 后运行 HPO，不读取 final test |
