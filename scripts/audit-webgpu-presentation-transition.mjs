...
const DEFAULT_OUTPUT_DIR = "/tmp/objgauss-webgpu-presentation-transition";
const DEFAULT_ASSETS = [
  "nerf-lego-alpha-closure-local",
  "plush-semantic-closure-local",
];
const MODE = "webgpu-presentation-transition-suite-v1";
...
try {
  if (assets.length === 0) {
    throw new Error("at least one asset is required for WebGPU presentation transition audit");
  }
  if (shouldStartServer) {
    if (!existsSync("dist/index.html")) {
      throw new Error("dist/index.html is missing; run `npm run build` before presentation transition audit");
    }
  ...
  for (const asset of assets) {
    console.log(`webgpu_presentation_transition_asset_start asset=${JSON.stringify(asset)}`);
    const result = await runTransitionProbe(asset);
    ...
    for (const check of row.checks) {
      console.log(
        `webgpu_presentation_transition_check=${check.passed ? "passed" : "failed"} ` +
    ...
    console.log(
      `webgpu_presentation_transition_asset=${row.passed ? "passed" : "failed"} ` +
        `asset=${JSON.stringify(asset)} firstFrame=${JSON.stringify(row.firstFrameStatus)}:${row.firstFramePixels} ` +
        `selected=${JSON.stringify(row.canvasSelectedObject)} visibleAfterIsolate=${JSON.stringify(row.visibleAfterIsolate)} ` +
        `visibleAfterDelete=${JSON.stringify(row.visibleAfterDelete)} postDelete=${JSON.stringify(row.postDeleteRendererId)}:${JSON.stringify(row.postDeleteObjectFilter)} ` +
  ...
  console.log(
    `webgpu_presentation_transition=${passed ? "passed" : "failed"} ` +
...
async function runTransitionProbe(asset) {
  const commandArgs = [
...
function parseAuditOutput(output) {
  ...
  const postDelete = line.match(/postDelete="([^"]*)":"([^"]*)":([^\s]+)/);
  ...
  return {
    ...
    initial: storageTiming,
    isolate: storageTimingAfterIsolate,
    delete: storageTimingAfterDelete,
    ...
    canvasSelectedObject: stringMatch(line, /canvasSelectedObject=([^\s]+)/),
    ...
    postDeleteRendererId: postDelete?.[1] ?? "",
    postDeleteObjectFilter: postDelete?.[2] ?? "",
    postDeleteSparkGaussians: postDelete?.[3] ?? "",
...
function buildRow({ asset, exitCode, parsed }) {
  const checks = [
    ...
    check("object-filter-target", parsed.objectFilterTarget, "gpu-object-state-buffer", parsed.objectFilterTarget === "gpu-object-state-buffer"),
    check("object-state-initial", parsed.objectStateChecksum, "non-empty", Boolean(parsed.objectStateChecksum)),
    check(
      "object-state-isolate-changed",
      `${parsed.objectStateChecksum}->${parsed.objectStateAfterIsolateChecksum}`,
      "changed",
      Boolean(parsed.objectStateAfterIsolateChecksum) &&
        parsed.objectStateAfterIsolateChecksum !== parsed.objectStateChecksum,
    ),
    check(
      "object-state-delete-changed",
      `${parsed.objectStateAfterIsolateChecksum}->${parsed.objectStateAfterDeleteChecksum}`,
      "changed",
      Boolean(parsed.objectStateAfterDeleteChecksum) &&
        parsed.objectStateAfterDeleteChecksum !== parsed.objectStateAfterIsolateChecksum,
    ),
    check("canvas-selected-object", parsed.canvasSelectedObject, "object id", isObjectId(parsed.canvasSelectedObject)),
    check("visible-after-isolate", parsed.visibleAfterIsolate, ">0", numericString(parsed.visibleAfterIsolate) > 0),
    check("visible-after-delete", parsed.visibleAfterDelete, ">0", numericString(parsed.visibleAfterDelete) > 0),
    check("deleted-objects", parsed.deletedObjects, "1", parsed.deletedObjects === "1"),
    check("render-mode-after-delete", parsed.renderModeAfterDelete, "原始颜色（编辑预览）", parsed.renderModeAfterDelete === "原始颜色（编辑预览）"),
    check("post-delete-renderer", parsed.postDeleteRendererId, "webgpu-tile", parsed.postDeleteRendererId === "webgpu-tile"),
    check("post-delete-filter", parsed.postDeleteObjectFilter, "gpu-object-state-buffer", parsed.postDeleteObjectFilter === "gpu-object-state-buffer"),
...
function checkTiming(label, timing, { allowed, queueDoneMayBePending = false }) {
  const timingBudget = timing.mode === "full-upload" ? maxFullUploadUpdateMs : maxObjectStateUpdateMs;
...
function renderMarkdown(summary) {
  const lines = [
    "# WebGPU Presentation Transition Suite",
    "",
    `Mode: \`${summary.mode}\``,
    ...
    "",
    "This gate proves headed full-canvas WebGPU presentation can survive object select, isolate, and delete transitions while staying on the C-path. It is not an FPS benchmark or a 1M interactive SLA.",
    "",
    "| Asset | Passed | Gaussians | Tile refs | Selected | Visible isolate/delete | Initial | Isolate | Delete | Screenshot |",
    "| --- | --- | ---: | ---: | --- | --- | --- | --- | --- | --- |",
  ];
  for (const row of summary.rows) {
    lines.push(
      `| ${escapeMarkdown(row.asset)} | ${row.passed ? "yes" : "no"} | ${row.packedGaussians} | ${row.tileReferences} | ${escapeMarkdown(row.canvasSelectedObject)} | ${escapeMarkdown(`${row.visibleAfterIsolate}/${row.visibleAfterDelete}`)} | ${escapeMarkdown(formatTiming(row.initial))} | ${escapeMarkdown(formatTiming(row.isolate))} | ${escapeMarkdown(formatTiming(row.delete))} | ${escapeMarkdown(row.screenshotPath)} |`,
    );
  }
