...
def _policy_ranking(variants: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    candidate_rows = [
        item for item in raw_candidate_rows if _clears_full_perturbation_gate(item)
    ]
    native_candidates = [
    ...
    return {
        ...
        "raw_candidate_policy_count": int(len(raw_candidate_rows)),
        "perturbation_rejected_policies": [
            str(item["policy"])
            for item in raw_candidate_rows
            if item not in candidate_rows
        ],
...
def _clears_full_perturbation_gate(variant: Mapping[str, Any]) -> bool:
    thresholds = ObjectStateModelIdentityGateThresholds().as_dict()
    breakdown = variant.get("perturbation_breakdown")
    if not isinstance(breakdown, Mapping) or not breakdown:
        return False
    for row in breakdown.values():
        if not isinstance(row, Mapping):
            return False
        metrics = row.get("assignment_solver_metrics")
        if not isinstance(metrics, Mapping):
            return False
        if (
            float(metrics["identity_retrieval_at_1"])
            < thresholds["identity_retrieval_at_1_min"]
            or float(metrics["identity_margin"])
            <= thresholds["identity_margin_min"]
            or float(metrics["assignment_consistency"])
            < thresholds["assignment_consistency_min"]
            or float(metrics["objectstate_drift"])
            > thresholds["objectstate_drift_max"]
        ):
            return False
    return True
