...
| R-002 | 当前默认对象分组不是端到端语义级对象分割 | 对象边界和语义一致性不稳定 | 已落地 `DEMO-001`：真实 Plush splat + Object Field + 多视角 mask voting + 前端对象编辑闭环可复现；`VERIFY-001` 已固化机器验收；`MASK-001` 可从 NeRF Lego 真实 RGBA 图片生成前景 mask manifest；默认 KMeans 仍保留为 baseline | 仓库内可对真实小场景生成 SAM / CLIP mask manifest，并与 KMeans 基线对比 | open |
