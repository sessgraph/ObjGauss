    ...
    return (
        ObjectStatePublicDatasetCandidate(
            ...
            next_actions=(
                ...
                "use the existing BOP capture adapter to produce controlled manifest rows",
        ...
        ObjectStatePublicDatasetCandidate(
            ...
            next_actions=(
                ...
                "use the existing BOP adapter to map pose/mask fields into controlled capture annotations",
        ...
        ObjectStatePublicDatasetCandidate(
            ...
            recommended_role=(
                "interaction pose/visibility stress candidate; no native action GT"
            ),
            ...
            blockers=(
                ...
                "egocentric fisheye/multi-stream format is not directly consumable by the existing normalized CSV importer",
                "released clip annotations do not include action labels, intervals, or control vectors",
                "observed manipulation footage is not action ground truth or counterfactual evidence",
                "no Gaussian reconstruction evidence until local conversion is done",
            ),
            next_actions=(
                "use only for identity/prediction after populating the existing normalized CSV and controlled capture contract",
                "keep intervention blocked unless action/control ground truth is independently measured",
                "do not derive action vectors from hand, tool, or target-object pose deltas",
            ),
            source_notes=(
                ...
                "The released HOT3D-Clips format lists camera, hand, object, and clip metadata but no action annotation contract.",
            ),
        ),
        ObjectStatePublicDatasetCandidate(
            candidate_id="dexycb",
...
def objectstate_public_dataset_candidates_audit(
    ...
    payload = {
        ...
        "next_actions": (
            ...
            "populate the existing controlled capture contract from normalized RGB, pose, mask, camera, and frame-order rows; do not add a dataset-specific wrapper",
            "generate local per-frame Gaussian evidence under ignored outputs/",
            "run identity and prediction rows; no default candidate supplies native action GT",
            "keep intervention claims blocked until independently measured action/control evidence exists",
        ),
        "claim_policy": {
...
def objectstate_public_interaction_route_markdown(summary: Mapping[str, Any]) -> str:
    ...
    lines.append(
        "- Observed manipulation footage is not action ground truth or randomized counterfactual evidence."
    )
    return "\n".join(lines) + "\n"
...
def _hard_blockers(
    ...
    blockers = [
        ...
        "all public candidates must populate the existing controlled capture manifest contract; no new dataset wrapper is authorized",
        "intervention rows remain blocked until independently measured action/control evidence is available",
    ...
    if not any(candidate.has_action_gt for candidate in candidates):
        blockers.append("no public candidate with native action ground truth was found")
...
def _public_interaction_route_status(readiness: Mapping[str, bool]) -> str:
    if not readiness.get("candidate_is_public_interaction_dataset"):
    ...
    if not readiness.get("candidate_has_action_gt"):
...
def _public_interaction_route_blockers(
    ...
    if not readiness["candidate_has_action_gt"]:
        blockers.append("selected candidate does not provide native action ground truth")
...
def _public_interaction_route_next_actions(
    ...
    if not readiness["candidate_has_action_gt"]:
        actions.append(
            "keep intervention blocked or select a source with independently measured action/control ground truth"
        )
    if not readiness["dataset_root_present"]:
        actions.append(
