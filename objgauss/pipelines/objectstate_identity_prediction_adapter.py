...
def objectstate_identity_predictions_from_trainable_artifact(
    ...
    *,
    ...
    source: str = "trainable_kernel_objectstate_raw_track_adapter",
            ...
            predictions.append(
                {
                    ...
                    "predicted_identity": f"object-state-{state['id']}",
                    "predicted_position": state["centroid"].tolist(),
    ...
    if max_distance is not None:
        candidate["max_association_distance"] = max_distance
...
def _state(value: Any) -> dict[str, Any]:
    ...
    return {
        "id": int(state_id),
        "slot": _state_slot(item),
        "centroid": centroid,
        "confidence": confidence_value,
    }


def _state_slot(item: Mapping[str, Any]) -> int | None:
    slot = item.get("slot")
    if slot is None:
        return None
    if isinstance(slot, bool) or not isinstance(slot, int):
        raise ValueError("trainable artifact ObjectState slot must be an integer")
    if slot < 0:
        raise ValueError("trainable artifact ObjectState slot must be non-negative")
    return int(slot)
