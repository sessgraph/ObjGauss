...
| R-002 | 当前对象分组不是语义级对象分割 | 对象边界和语义一致性不稳定 | 已落地 `OBJFIELD-001`，用 `object_logits: (N,K)` 统一 soft slot 接口，并保留 KMeans warm start baseline；`SEG-001` 继续接语义指导 | 语义/实例分组方案输出 `object_id` 并有基准对比 | open |
