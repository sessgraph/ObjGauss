...
| PR-02C Trainer/Baselines | `c6_config_frozen_supported_c7_pending` | C0–C3 clean supported；C3 为 `c3_pushed_remote_cpu_supported_gpu_local_supported`。C6 contract/runner 为 `28d1b39`/`080d844`；远端 CPU run `29422872955` 成功；本地 clean HPO 为 24/24 tasks、12/12 pairs、0 retry、20/20 checks，semantic hash `33679c22…6d6d7f`；映射为 action-free `hpo-h064-lr0p0003`、action-conditioned `hpo-h128-lr0p0010` | 严格执行 C7：两个 selected configs × 3 seeds 从头 train-only formal training，validation-only checkpoint selection；不读取 final test，不复用 HPO checkpoint |
