...
| PR-02C 在 config/checkpoint 冻结前泄漏 final GT | mitigated_local | Owner 已决定 PR-02C 只物化 train/validation；source CLI、loader 与 verifier 对 test/future fail closed，trainer/HPO/selector 后续仍须重复防御 | `adb1a62` clean C1 无 test artifact，source/loader 双拒绝与 16 项独立 checks supported；后续 C2+ 仍须保持控制 |
...
| C2 producer 与 verifier 因对象键序或跨语言浮点算法产生伪一致/伪漂移 | active_pending_clean | prediction payload 使用 Node canonical JSON；verifier 使用同一 canonical 比较但独立重算状态数学，并让平方和/归一化运算顺序与规范一致；canonical/reverse index 与 120 份 predictions 必须字节级一致 | 首轮 diagnostic 正确暴露 verifier 的键序与 `hypot` 差异，修复后 18 checks 与 repeat supported；corruption mutation 被四层拒绝；仍待 clean HEAD acceptance |
| C2 baseline 偷读 source trajectory/future GT 或意外物化 final | active_pending_clean | C1 单独发布 sanitized validation bundle；baseline 进程只读取 bundle；独立 Node verifier 从 raw source 重算投影、禁止 future/executed/test，并核对完整文件集 | dirty diagnostic 60 branches / 120 predictions、18 checks supported；提交后仍须运行 `./scripts/check-pr02c-baselines` |
