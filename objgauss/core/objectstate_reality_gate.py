...
_REQUIRED_METRICS_BY_EVIDENCE = {
    "identity": ("idf1", "fragmentation_rate", "swap_rate", "identity_collapse"),
    "prediction": ("state_ade", "history_ade", "prediction_gap_vs_history_model"),
    "intervention": (
...
def validate_objectstate_reality_row(
    ...
    if row.source_kind == "open_world_real" and row.status == "pass":
        raise ValueError("open_world_real rows cannot be marked pass in this reality gate")
    ...
    if row.status == "fail" and not failure_reason:
        raise ValueError("failed reality rows require failure_reason")
    if row.status == "pass" and (block_reason or failure_reason):
        raise ValueError("pass reality rows cannot carry block_reason or failure_reason")
