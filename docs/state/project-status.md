...
- 前端:
  ...
  - RENDER-004 WebGPU tile renderer 已补 ADR 设计入口；RENDER-004A capability detection / renderer boundary 与 RENDER-004B buffer packing / binning smoke contract 已完成，当前 Gaussian OIT 明确作为 WebGPU fallback，下一步是 RENDER-004C tile accumulation / fullscreen resolve。
...
- RENDER-004 design: `docs/adr/0005-webgpu-tile-renderer.md` 已定义 WebGPU tile renderer 的 staged delivery、data contract、tile binning、per-tile accumulation、object-state buffer、fallback contract 和验收标准；下一步是 `RENDER-004B` Gaussian WebGPU buffer packing / binning smoke path。
