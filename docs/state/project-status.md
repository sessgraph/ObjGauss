...
- 前端:
  ...
  - RENDER-005B 已落地 WebGPU storage-buffer upload contract：WebGPU route 会创建并写入 `webgpu-tile-storage-v1` buffers，覆盖 Gaussian geometry/color/object-state/tile counts/tile accumulation/resolve payload；当前仍未实现 compute accumulation shader。
  - RENDER-005C 已落地 WebGPU storage-buffer resolve shader：WebGPU first-frame display path 会从 `tileResolvedRgba` storage buffer 读取并 fullscreen resolve；当前 `tileResolvedRgba` 内容仍来自 CPU smoke resolve，compute accumulation 仍是下一步。
  - RENDER-005D 已落地 WebGPU compute resolve shader：WebGPU route 会先 dispatch compute，把 `tileAccumulation` resolve 到 `tileResolvedRgba`，再由 storage-buffer fullscreen pass 显示；当前仍未实现 per-Gaussian tile traversal。
