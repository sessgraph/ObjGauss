...
- 前端开发服务固定使用 `127.0.0.1:5395`，不要因为端口占用临时轮换到其他端口。
- `npm run dev` 通过 `scripts/dev-server.mjs` 启动；如果 5395 已被本仓库的 ObjGauss
  Vite/dev server 占用，可以自动重启该本项目服务。
- 如果 5395 被非本项目进程占用，先报错并提醒 Owner，不要误杀外部进程。
- 浏览器验收、audit 和截图证据默认都应指向 5395；临时端口只能作为明确记录的异常排查手段。
