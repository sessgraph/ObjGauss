...
- 前端:
  ...
  - RENDER-005G 已落地 WebGPU viewport pixel output contract：WebGPU route 会把 tile accumulation resolve 到 `tileResolvedRgba`，再 dispatch `webgpu-compute-pixel-resolve-v1` 写入 viewport-sized `pixelResolvedRgba`，最后由 `webgpu-pixel-storage-resolve-v1` fullscreen shader 显示；当前仍未实现直接逐像素 Gaussian accumulation。
