...
_CLAIM_POLICY_KEYS = (
    ...
    "uses_exact_audited_teacher_evidence",
...
def objectstate_assignment_long_smoke_summary(
    ...
    *,
    ...
    teacher_evidence_batches: Sequence[TeacherEvidenceBatch] | None = None,
    ...
    _validate_report_ladder(source_scenarios)
    if teacher_evidence_batches is None:
        raise ValueError(
            "assignment long smoke requires the audited teacher evidence batches"
        )
    scenario_list, audited_evidence = teacher_evidence_scenarios_from_audit(
        source_scenarios,
        tuple(teacher_evidence_batches),
        teacher_evidence_leakage_audit,
    )
    ...
    payload = {
        ...
        "contract": contract,
        "teacher_evidence": {
            "content_sha256": audited_evidence["content_sha256"],
            "binding_policy": audited_evidence["binding_policy"],
            "batch_count": audited_evidence["batch_count"],
            "scenario_count": audited_evidence["scenario_count"],
            "row_count": audited_evidence["row_count"],
            "feature_dim": audited_evidence["feature_dim"],
            "sources": sorted(
                {item["source"] for item in audited_evidence["batch_digests"]}
            ),
            "matches_passed_leakage_audit": True,
            "used_for_assignment_training": True,
            "used_for_identity_evaluation": True,
        },
        "run_config": {
            "evidence_policy": "semantic",
            "teacher_feature_dim": int(audited_evidence["feature_dim"]),
            "iterations": int(iterations),
            "learning_rate": float(learning_rate),
            "seed": int(seed),
...
def validate_objectstate_assignment_long_smoke_summary(
    ...
    _validate_teacher_evidence_digest(_mapping(payload, "teacher_evidence"))
    ...
    if _positive_int(config.get("iterations"), "iterations") > 600:
        raise ValueError("assignment long smoke iterations must stay <= 600")
    if _positive_int(config.get("teacher_feature_dim"), "teacher_feature_dim") != int(
        payload["teacher_evidence"]["feature_dim"]
    ):
        raise ValueError("assignment long smoke teacher feature_dim mismatch")
...
def _validate_teacher_evidence_digest(payload: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise ValueError("assignment long smoke teacher evidence sha256 invalid")
    if payload.get("binding_policy") != "scenario_order_frame0_then_frame1":
        raise ValueError("assignment long smoke teacher evidence binding policy mismatch")
    for key in ("batch_count", "scenario_count", "row_count", "feature_dim"):
        _positive_int(payload.get(key), f"teacher_evidence.{key}")
    ...
    if (
        ...
        raise ValueError("assignment long smoke teacher evidence sources invalid")
    for key in (
        "matches_passed_leakage_audit",
        "used_for_assignment_training",
        "used_for_identity_evaluation",
    ):
        if payload.get(key) is not True:
            raise ValueError(f"assignment long smoke teacher evidence must set {key}")
