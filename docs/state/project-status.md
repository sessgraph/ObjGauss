...
guard 已实现。正式 spec 不再硬编码旧提交，而由运行时注入当前 clean HEAD；当前 worktree
尚未提交，所以 builder/verifier 正确拒绝生成最终 SHA 证据。A–F 均未运行远端 CI，PR-01 仍未
关闭；下一门是 Owner 授权提交、最终 clean-checkout 全量复跑和远端 CI。
