...
def _progress_next_actions(
    ...
    if not readiness["controlled_bundle_intervention_action_gt_ready"]:
        return [
            "supply independently measured non-zero action vectors in actions.csv whose "
            "time intervals cover object pose transitions; never derive them from "
            "target-object pose deltas"
        ]
