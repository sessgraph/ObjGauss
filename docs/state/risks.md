...
| R-017 | ObjectState 可能只是 observation slot，不是 persistent world state | 三段 BOP 为 0 pass / 6 fail / 3 blocked；首批三段 RBO 已闭环 measured action 并形成 reviewable 0 pass / 9 fail / 0 blocked，但严格 V-O-V 仍为 `0/3`，follow-up P0 也未解锁 scene；RRC 不适配现行 3D action | 至少 3 个严格 controlled scenes 独立完成 identity/prediction/intervention 比较，并超过简单 baseline | open / P0 |
