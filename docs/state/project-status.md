...
- 前端:
  ...
  - RENDER-ROUTE-013 已新增 WebGPU C-path readiness audit：`npm run audit:webgpu-cpath-readiness` 会串起 build、1M scale budget、1M edit-cost budget 和 fixed-port headed presentation transition，输出 combined evidence / remaining gap report。当前本机通过：1M scale budget max/total=`122.07/173.24 MiB`，1M edit upload=`4 KiB`，headed browser 最大场景=`281498` Gaussians，max queue done=`1836.8ms`；报告明确保留 `browserRuntime1m=not-proven` 与 `fpsSla=not-proven`。
