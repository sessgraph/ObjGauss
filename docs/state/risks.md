...
| R-018 | Semantic / teacher evidence 可能泄漏 GT identity | 如果 semantic feature 由 `physical_identity_label`、`target_assignment`、oracle object id 或测试标签 one-hot 构造，identity retrieval@1 可能只是答案泄漏，不证明 inference-time evidence 支撑 ObjectState identity | `OBJECTSTATE-IDENTITY-GATE-POLICY-SCOPING-001` 已把当前 `candidate_ready` 限定为 policy=`semantic`，并明确 native Gaussian long-training gate blocked；后续必须通过 Teacher Evidence Contract 和 Leakage Audit 才能把 semantic evidence 用作训练主线 | `OBJECTSTATE-TEACHER-EVIDENCE-CONTRACT-001` 定义 provenance / allowed source / forbidden source，且 `OBJECTSTATE-TEACHER-EVIDENCE-LEAKAGE-AUDIT-001` 覆盖 semantic shuffle、random semantic baseline、physical label ban 和 train/test source split | open |
