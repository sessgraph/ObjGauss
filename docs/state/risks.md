...
| PR-02C 在 config/checkpoint 冻结前泄漏 final GT | active_control_implemented | Owner 已决定 PR-02C 只物化 train/validation；source CLI、loader 与 verifier 对 test/future fail closed，trainer/HPO/selector 后续仍须重复防御 | C1 dirty 60-group 诊断无 test artifact，source/loader 双拒绝与独立 checks 通过；仍待 clean evidence |
| C1 source checksum/lineage 漂移或跨 split identity 复用 | active_control_implemented | 冻结 formal spec hash；producer、loader、独立 verifier 三方重算 data index；完整 group/branch/checksum/source-plan/HEAD 与初始化 lineage 检查 | dirty 300-branch 诊断三方 index `ce6e6bd9…20c209` 一致；仍待 clean evidence |
