...
相对首个 pose 最大平移为 `0.109–0.173m`。权威 link pose 使用 base marker → `rb0`、
同刻 `joint_states` 与 matching-date URDF。严格逐 link RGB-D/mesh z-buffer 重算没有任何
一段形成 clear → `occlusion_fraction >= 0.5` → clear；逐帧最大遮挡为
`0.421140/0.467351/0.476498`。

`ftSensor` model 与 ATI sign 审计仍支持 `/map`、N 的
`measured_tool_on_object_force`，且禁止 `action_delta`。official-chain mesh/link 重算接受
`5/8` validation windows 与 `3/7` 独立 50 Hz intervals；三条 interval 共覆盖 tripod/cabinet
两个 scenes、使用 `1,172/1,172` 条 raw samples，cardboard 没有合格 interval。三个重建
3-frame bundle 的 capture/file validation 通过，cardboard action 被明确移除；旧
direct-marker `5 intervals / 3 bundles / 0-9-0 ledger` 已 supersede。新 canonical ledger 为
`0 pass / 7 fail / 2 blocked`；cardboard 的独立 prediction-only eval 为 fail，但 full
handoff 因无 action 不可运行，主 ledger 不手工合并该结果并保持 prediction/intervention
blocked；tripod/cabinet 的三类 reality rows 均 fail。三段仍因没有严格 V-O-V、只有一种
真实 lighting 而不能计作合格 controlled scene。一次性复核产物位于 ignored
...
- `treasurebox25` 非启动帧 `553/553`，base 最大遮挡 `0.443408`，moving link 最大
  `0.193093`，均无 V-O-V，是可信负证据；
- `globe25` 非启动帧 `373/373`，official chain 已解决 moving-link 失配；base/moving 最大
  遮挡为 `0.427475/0.048164`，均无 V-O-V，是可信负证据；
