...
| PR-02 训练抢占桌面显示显存 | active | 12 GiB 进程 cap 与 1 GiB 实际空闲保留取更严者；不足即 blocked | C3 `4498bd6` clean golden 训练峰值 68,277,760 bytes、最低空闲 15,633,416,192 bytes，门已 supported；仍须在 HPO/formal 持续监控 |
...
| PR-02C 在 config/checkpoint 冻结前泄漏 final GT | mitigated_local | Owner 已决定 PR-02C 只物化 train/validation；source CLI、loader、trainer 与 verifier 对 test/future fail closed，HPO/selector 后续仍须重复防御 | C3 `4498bd6` clean gate 无 test artifact，test split 以 exit 4 被拒绝；后续 HPO/selector 仍须复核 |
...
| Action-free 与 action-conditioned 容量或训练预算不公平 | mitigated_local | 共享 backbone/action encoder、参数量、updates、数据顺序、grid 和 seeds；独立 verifier 重算 checkpoint structure 与账本；HPO 对 validation groups/3 seeds 两级等权 | C3 `4498bd6` clean golden 两 arm 各 35,734 参数且 24 checks supported，篡改参数账本被 exit 4 拒绝；24-task HPO/6-task formal audit 尚未运行 |
| Variable-`Δt` rollout 静默变成按时刻独立 heads 或读取 executed/GT | mitigated_local | 四区间共享 transition；显式 `Δt`、区间裁剪 commanded schedule 与 COM 作用点；初态后无 teacher forcing | C3 `4498bd6` unit/CPU tiny/GPU clean repeat 覆盖 `0.1/0.1/0.3/0.6 s`、共享参数和 feature visibility；不能外推为模型性能 |
