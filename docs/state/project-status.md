...
- 前端:
  ...
  - RENDER-ROUTE-011 已将 WebGPU presentation smoke 纳入 product renderer acceptance：`npm run acceptance:renderer-product` 现在按 route contract -> build -> headed WebGPU presentation performance -> Spark commercial route 的顺序验收；CI profile 仍不跑 headed presentation，继续保持 fresh-clone/headless 友好。
