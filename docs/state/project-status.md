...
1. RENDER-005P: 将 fullscreen pixel-storage display pass 替换/对照为 texture-backed display probes，验证 sampled texture / copy-buffer-to-texture / storage texture 输出能否绕开当前 headless unsafe WebGPU backend loss；同时在 WebGPU-capable 桌面 Chrome 中重跑 Plush / Lego first-frame runtime audit。
