        ...
        "state_placeholders": [
            {
                ...
                "persistent_id": "TODO integer candidate persistent identity",
                "slot": "TODO integer renderer slot / object_id",
                "state_id": None,
                ...
                "note": (
                    "Do not copy BOP pose GT values into this template. "
                    "persistent_id is identity; slot is the renderer address; "
                    "state_id is a legacy-only fallback."
                ),
            }
            for item in objects
        ],
...
def _validate_template_frame(frame: Any, *, expected_index: int) -> None:
    ...
    for item in placeholders:
        ...
        for key in ("centroid", "bbox", "confidence"):
            if key not in item:
                raise ValueError(
                    f"BOP candidate artifact template placeholder requires {key}"
                )
        has_persistent_id = "persistent_id" in item
        has_slot = "slot" in item
        if has_persistent_id != has_slot:
            raise ValueError(
                "BOP candidate artifact template placeholder requires both "
                "persistent_id and slot"
            )
        if not has_persistent_id and "state_id" not in item:
            raise ValueError(
                "BOP candidate artifact template placeholder requires persistent_id "
                "and slot (or legacy state_id)"
            )
        note = item.get("note")
    ...
    *,
    ...
    for frame_index, (template_frame, capture_frame) in enumerate(
        ...
        for placeholder in template_frame["state_placeholders"]:
            ...
            if state["persistent_id"] in persistent_ids:
                raise ValueError(
                    "BOP candidate artifact finalizer requires unique persistent_id per frame"
                )
            if state["slot"] in slots:
                raise ValueError(
                    "BOP candidate artifact finalizer requires unique slot per frame"
                )
            persistent_ids.add(state["persistent_id"])
        ...
        expected_slots = set(range(len(states)))
        if slots != expected_slots:
            raise ValueError(
                "BOP candidate artifact finalizer slots must cover [0, state_count)"
            )
        object_state_frames.append(
            {
                "frame_index": frame_index,
                "states": states,
                "derived_object_ids": [int(state["slot"]) for state in states],
            }
        )
        assignments.append(
            _slot_assignment(frame_index, [int(state["slot"]) for state in states])
        )
    artifact = {
...
def _state_from_placeholder(
    ...
    *,
    ...
    explicit_addresses_ready = _is_integer(placeholder.get("persistent_id")) and _is_integer(
        placeholder.get("slot")
    )
    legacy_address_ready = _is_integer(placeholder.get("state_id"))
    if explicit_addresses_ready:
        _reject_todo(placeholder, f"frame {frame_index} placeholder")
        persistent_id = _non_negative_integer(
            placeholder["persistent_id"],
            "persistent_id",
        )
        slot = _non_negative_integer(placeholder["slot"], "slot")
        address_diagnostic = "explicit_persistent_id_and_renderer_slot"
    elif legacy_address_ready:
        for key in ("centroid", "bbox", "confidence"):
            _reject_todo(
                placeholder[key],
                f"frame {frame_index} placeholder.{key}",
            )
        legacy_state_id = _non_negative_integer(placeholder["state_id"], "state_id")
        persistent_id = legacy_state_id
        slot = legacy_state_id
        address_diagnostic = "legacy_state_id_used_for_identity_and_renderer_slot"
    else:
        _reject_todo(placeholder, f"frame {frame_index} placeholder")
        raise ValueError(
            "BOP candidate artifact placeholder requires integer persistent_id and slot"
        )
    ...
    return {
        ...
        "status": "active",
        "diagnostics": [
            "bop_candidate_finalized_from_template",
            address_diagnostic,
        ],
    }


def _slot_assignment(frame_index: int, slots: list[int]) -> dict[str, Any]:
    state_count = len(slots)
    return {
        "frame_index": frame_index,
        "shape": [state_count, state_count],
        "matrix": [
            [
                1.0 if slots[row] == column else 0.0
