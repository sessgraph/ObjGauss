...
def test_core_namespace_exposes_v2_stability_foundation_contract():
    ...
    assert OBJECTSTATE_REAL_INTERVENTION_ROWS_SCHEMA == (
    ...
    assert objectstate_real_intervention_rows_from_bundle is not None
    assert objectstate_real_intervention_rows_summary is not None
    ...
    assert OBJECTSTATE_TEMPORAL_ASSIGNMENT_REQUIRED_POLICY == "semantic"
    assert "assignment_t" in OBJECTSTATE_TEMPORAL_ASSIGNMENT_INPUTS
    assert "assignment_consistency" in OBJECTSTATE_TEMPORAL_ASSIGNMENT_LOSS_TERMS
    assert "temporal_assignment_consistency" in OBJECTSTATE_TEMPORAL_ASSIGNMENT_METRICS
    assert ObjectStateTemporalAssignmentContractThresholds().as_dict()[
        "max_duration_seconds"
    ] == 600
    ...
    temporal_contract = objectstate_temporal_assignment_contract_summary()
    assert validate_objectstate_temporal_assignment_contract_summary(
        temporal_contract
    ) == temporal_contract
    assert (
        temporal_contract["readiness_gate"]["temporal_assignment_contract_ready"]
        is False
    )
    ...
    assert OBJECTSTATE_TRANSITION_INTERVENTION_CANDIDATES_SCHEMA == (
        "objgauss-objectstate-transition-intervention-candidates-v1"
    )
    assert "action_delta" in OBJECTSTATE_TRANSITION_INTERVENTION_POLICIES
    ...
    assert objectstate_transition_intervention_candidates_summary is not None
    ...
    assert write_objectstate_transition_intervention_candidates is not None
    ...
    assert validate_objectstate_transition_intervention_candidates_summary is not None
    ...
    transition_interventions = objectstate_transition_intervention_candidates_summary(
        transition_dataset,
    )
    assert transition_interventions["schema"] == (
        OBJECTSTATE_TRANSITION_INTERVENTION_CANDIDATES_SCHEMA
    )
    assert validate_objectstate_transition_intervention_candidates_summary(
    ...
    assert validate_objectstate_public_interaction_workspace_progress_summary is not None
    ...
    assert objectstate_public_interaction_workspace_progress is not None
    ...
    assert OBJECTSTATE_PUBLIC_INTERACTION_WORKSPACE_PROGRESS_SCHEMA == (
